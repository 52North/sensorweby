...
shinyServer(function(input, output, session) {
    output$timePlot<- renderPlot({
        ...
        # We can't simply use `sensorweb4R::id(input$series)` as pollutant names,
        # as the IDs may be integers that will be transformed to `X<id>` when we
        # create the `data.frame` and so can not be found by `timePlot`. So to
        # be safe use the names generated by the `data.frame`:
        pollutants <- names(df)[!(names(df) %in% "date")]

        timePlot(df, pollutant = pollutants,
