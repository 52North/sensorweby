...
shinyServer(function(input, output, session) {
    output$scatterPlot<- renderPlot({
        ...
        scatterPlot(df, x = sensorweb4R::id(input$series)[1], y = sensorweb4R::id(input$series)[2], xlab = label(input$series)[1], ylab = label(input$series)[2], method = "density", col= "jet")
    })
})
